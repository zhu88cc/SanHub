...
export default function ModelsPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-light text-white">聊天模型</h1>
        <p className="text-white/50 mt-1">配置 OpenAI 兼容的聊天模型</p>
      </div>
      ...
      <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 space-y-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            <label className="text-sm text-white/70">最大 Tokens</label>
            <input
              type="number"
        ...
        <div className="flex items-center gap-6 pt-2">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-white/70">支持图片输入</span>
          </label>
          <label className="flex items-center gap-2 cursor-pointer">
            <input
              type="checkbox"
        ...
        <div className="flex items-center gap-3 pt-4">
          <button
            onClick={saveModel}
            disabled={saving}
            className="flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-violet-500 to-fuchsia-500 text-white rounded-xl font-medium hover:opacity-90 disabled:opacity-50"
          >
          ...
          {editingId && (
            <button
              onClick={resetForm}
              className="px-5 py-2.5 bg-white/10 text-white rounded-xl hover:bg-white/20"
            >
              取消
            </button>
          )}
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b border-white/10">
              <th className="text-left text-sm font-medium text-white/50 px-5 py-4">名称</th>
              <th className="text-left text-sm font-medium text-white/50 px-5 py-4">模型 ID</th>
              <th className="text-left text-sm font-medium text-white/50 px-5 py-4">Base URL</th>
              <th className="text-center text-sm font-medium text-white/50 px-5 py-4">图片</th>
          ...
          <tbody>
            {models.map((model) => (
              <tr key={model.id} className="border-b border-white/5 hover:bg-white/[0.02]">
                <td className="px-5 py-4 text-white font-medium">{model.name}</td>
                <td className="px-5 py-4 text-white/60 font-mono text-sm">{model.modelId}</td>
                <td className="px-5 py-4 text-white/60 text-sm truncate max-w-[200px]">{model.apiUrl}</td>
